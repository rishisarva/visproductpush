...
def snapshot_products(products: list[dict]) -> None:
    """
    ...
    # upsert in batches; on conflict the row is replaced
    ...
    existing = _call("GET", "sync_products?select=sku") or []
    gone = [r["sku"] for r in existing if r.get("sku") and r["sku"] not in live]
    for sku in gone:
